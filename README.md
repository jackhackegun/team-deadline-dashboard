...
| 환경변수 | 기본값 | 설명 |
|---|---|---|
| `GITHUB_TOKEN` | 없음 | **팀으로 쓸 거면 사실상 필수.** 없으면 시간당 60회 제한 |
| `GITHUB_SYNC_MINUTES` | `5` | 커밋 확인 주기(분) |
| `GITHUB_COMMIT_LOOKBACK_DAYS` | `30` | 며칠 전 커밋까지 훑을지 |

### 깃허브 요청 한도 — 팀으로 쓸 땐 토큰을 넣으세요

토큰이 없으면 깃허브는 **시간당 60회**만 허용합니다. 레포 5개를 5분마다 확인하면
그것만으로 한도를 다 씁니다. 그러면 오가니제이션 연결도, 커밋 확인도 전부 막힙니다.

```
깃허브 요청 한도(시간당 60회)를 모두 썼습니다. 9분 뒤 풀립니다.
```

**토큰을 넣으면 시간당 5,000회**가 됩니다.

1. https://github.com/settings/tokens 접속
2. **Generate new token (classic)** 클릭
3. Note에 아무 이름(예: `team-dashboard`), Expiration 선택
4. 권한(scope) 선택:
   - **공개 레포만 쓴다면 아무것도 체크하지 마세요.** 체크 없이도 한도는 5,000회로 늘어납니다
   - 비공개 레포를 읽어야 하면 `repo`만 체크
5. 생성된 `ghp_...` 값을 `.env`에 넣고 다시 띄웁니다
...
필요 없는 권한은 주지 마세요. 공개 레포라면 **권한 0개짜리 토큰으로 충분합니다.**
